...
本次 Windows 实测见 [performance.json](performance.json)：总墙钟 `11.201268s`，峰值 RSS `323964928` 字节，结果已持久化，低于 `90s` 和 `4 GiB` 门槛。当前 Docker Desktop 守护进程不可用，因此 4 GiB 结论来自全程进程峰值观测，不宣称施加了容器硬内存上限；后续任务 20 容器验证应在 Docker 可用时补充硬限额证据。
